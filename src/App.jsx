...
function App() {
  ...
  const [isQualificationValid,setIsQualificationValid] = useState(false)
  ...
  const userInputValidation = (inputData) => {
    ...
    if (name == "fullName") {
      ...
      value.length < 0 ? setIsQualificationValid(true) : setIsQualificationValid(false)
    }
    else if (name === "gender") {
      setGender(value);
      ...
      <div className='container-fluid d-flex justify-content-center align-items-center vh-100 w-100 bg-primary'>
        <div style={{ width: '600px' }} className='bg-light rounded p-5'>
          ...
          <form className='mt-3' >
            ...
            <div className='mb-3'>
              <TextField value={qualification || ""} onChange={e => userInputValidation(e.target)} name='qualification' className='w-100' id="outlinedqualification" color='dark' type='text' label="Qualification" variant="outlined" />
              {
                isQualificationValid && (
                  <div className='text-danger'>*Enter valid Details</div>
                )
              }
            </div>
            <div className='mb-3'>
            ...
            <div className='mb-3 text-center'>
              <Button onClick={handleSubmit} disabled={!isFormValid} style={{ width: '150px', height: '50px', marginRight: '20px',  backgroundColor: isFormValid ? 'green' : 'grey' }} variant="contained">Submit</Button>
              <Button onClick={handleReset} style={{ width: '150px', height: '50px', backgroundColor: 'red' }} variant="contained">Cancel</Button>
            </div>
          </form>
        </div>
